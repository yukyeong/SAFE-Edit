...
CANDIDATE_HEADS="outputs/attention_heads_edit/heads/${ATTN_RUN}_top256.json"
SCREEN_RUN="${RUN_NAME}_post_ffn_edit16_causalcv"
HEAD_CONFIG="outputs/attention_heads_edit/heads/${SCREEN_RUN}_causalcv_top3_alpha0p02.json"
...
mkdir -p logs/ffn_edit outputs/logs/manifests outputs/attention_heads_edit/heads outputs/ffn_edit/metrics "${OUTPUT_ROOT}" "${SUMMARY_DIR}"
...
wait_gpu_idle
rm -f "${HEAD_CONFIG}"
"${PYBIN}" -m attention_heads_edit.locate.select_attention_heads_causal_cv_after_ffn_edit \
  --dataset "${TRAIN_DS}" \
