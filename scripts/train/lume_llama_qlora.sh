...
cd "$1"
python -u -m ffn_edit.train.accelerate_cli launch \
  --num_processes 1 \
  --num_machines 1 \
...
' _ "${REPO_ROOT}/src/ffn_edit/train" "${LLAMA_MODEL_PATH}" "${LLAMA_TRAIN_JSON}" "${LLAMA_VAL_JSON}" "${LLAMA_OUTPUT_DIR}" "${LLAMA_FINAL_MODEL_DIR}" </dev/null >> "${LLAMA_LOG_FILE}" 2>&1 &
