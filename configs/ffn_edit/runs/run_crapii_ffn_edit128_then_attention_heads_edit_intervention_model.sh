...
ATTRIBUTION_SCRIPT="srcs/ffn_edit/eval/compact_attribution_gemma.py"
...
case "${MODEL_KIND}" in
  llama)
    ...
    ATTRIBUTION_SCRIPT="srcs/ffn_edit/eval/compact_attribution_llama.py"
    ATTRIBUTION_EXTRA_ARGS=()
...
CANDIDATE_HEADS="${CANDIDATE_HEADS:-configs/attention_heads_edit/runs/${ATTN_RUN}_top${CANDIDATE_TOP_K}.json}"
HEAD_CONFIG="${HEAD_CONFIG:-configs/attention_heads_edit/runs/${SCREEN_RUN}_exactsafe_top${SELECT_TOP_K}_alpha${SAFE_ALPHA}.json}"
...
mkdir -p logs/ffn_edit configs/ffn_edit/runs configs/attention_heads_edit/runs "${RUN_ROOT}" "${SUMMARY_DIR}" "${ATTR_DIR}"
...
if [ ! -s "${CANDIDATE_HEADS}" ]; then
  ...
  wait_gpu_idle
  "${PYTHON_BIN}" srcs/attention_heads_edit/scripts/locate_api4_attention_heads.py \
...
"${PYTHON_BIN}" srcs/attention_heads_edit/scripts/select_attention_heads_by_exact_after_ffn_edit.py \
